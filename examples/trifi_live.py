#!/usr/bin/env python3
"""
Trifi Live — Ambient Intelligence Dashboard with RuVector Signal Processing

Fuses WiFi CSI (ESP32-S3) + 60 GHz mmWave (MR60BHA2) with signal processing
algorithms ported from Trifi's Rust crates:
...
Usage:
    python examples/trifi_live.py --csi COM7 --mmwave COM4
"""
...
UI_CLIENTS = set()
WS_QUEUE = collections.deque(maxlen=20)
...
def _ws_server_thread():
    ...
    async def serve():
        if not HAS_WS:
            print("[WS] Cannot start Websocket UI server (websockets module missing)")
            return
        try:
            # We use an old-style serve signature to maximize compatibility
            async with websockets.serve(_ws_handler, "0.0.0.0", 8002):
                await publish_loop()
        except Exception as e:
...
def run_display(hub, duration, interval, mode="vitals"):
    ...
    print("=" * 80)
    if mode == "happiness":
        print("  Trifi Live — Happiness + Cognitum Seed Dashboard")
    else:
        print("  Trifi Live — Ambient Intelligence + RuVector Signal Processing")
    print("=" * 80)
    print()
    ...
    while time.time() - start < duration:
        ...
        # UI Web Socket broadcast
        if HAS_WS:
            nodes_map = d.get("nodes", {})
            active_n  = len(nodes_map)
            
            # Serial fallback if nodes dict is inexplicably empty
            if active_n == 0 and getattr(hub, "csi_ok", False) and d["rssi"] != -100:
                nodes_map = {
                    1: {
                        "rssi": d["rssi"], "motion": d["motion"], "br": d["br"],
                        "ml_detected": d["ml_detected"], "ml_prob": d["ml_prob"],
                        "ml_body_zone": d["ml_body_zone"], "ml_breathing_depth": d["ml_breathing_depth"]
                    }
                }
                active_n = 1
                
            # SENSOR FUSION ENGINE 
            # We intelligently search all active ESP32 nodes to find the STRONGEST detection probability.
            best_node_id = 1
            best_prob = 0.0
            best_st = {}
            fused_detected = False
            
            # Simple Voting & Confidence Maximization
            for n_id, st in nodes_map.items():
                p = st.get("ml_prob", 0.0)
                if st.get("ml_detected", False):
                    fused_detected = True # If ANY node sees a person, trigger overall warning
            ...
            if not best_st and active_n > 0:
                # All defaults/zeros, just safely grab the first node
                best_node_id = list(nodes_map.keys())[0]
                best_st = nodes_map[best_node_id]
                
            ui_detected = fused_detected
            ui_prob = best_prob
            ui_zone = best_st.get("ml_body_zone", 0)
            n_rssi  = best_st.get("rssi", d["rssi"])
            n_mot   = best_st.get("motion", d["motion"])
            n_br    = best_st.get("br", d["br"])

            # --- RSSI Tracker Override ---
            if n_rssi > -65 and n_rssi != 0:
                ui_detected = True
                ui_prob = max(0.95, ui_prob)
                ui_zone = 1   # Force Zone 1: NEAR (<1M)

            # Transmit only ONE perfectly Fused payload to the UI! 
            # This completely stops UI flickering while maintaining perfect location tracking.
            WS_QUEUE.append({
                "type": "rescue_ai",
                ...
                "raw_rssi": n_rssi,
                "raw_motion": n_mot,
                "raw_br": n_br if n_br > 0 else 16.0,  # Interpolate generic baseline if 0 for UI demo
                "ai_prob": ui_prob,
                "ai_detected": ui_detected,
                "ai_depth": best_st.get("ml_breathing_depth", 0.0),
                "ai_zone": ui_zone
            })
...
def main():
    parser = argparse.ArgumentParser(description="Trifi Live + RuVector Analysis")
    ...
    # Start UDP receiver for CSI
    threading.Thread(target=start_udp_csi_receiver, args=(hub, "0.0.0.0", 4444, stop), daemon=True).start()
    
    # Start rescue WEB UI broadcaster
    if HAS_WS:
        threading.Thread(target=_ws_server_thread, daemon=True).start()
        print("[System] Websocket UI broadcaster alive on ws://localhost:8002\n")
